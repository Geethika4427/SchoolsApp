  ...
  return (
    <div className={styles.formContainer}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={styles.formGroup}>
          <label className={styles.label}>Email</label>
          <input type="email" className={styles.input} {...register("email_id", { required: "Email is required" })} />
          {errors.email_id && <span>{errors.email_id.message}</span>}
        </div>
        ...
        <div className={styles.formGroup}>
          <label className={styles.label}>Contact</label>
          <input type="number" className={styles.input} {...register("contact", { required: "Contact is required" })} />
        </div>

        {/** Address **/}
        <div className={styles.formGroup}>
          <label className={styles.label}>Address</label>
          <input className={styles.input} {...register("address", { required: "Address is required" })} />
        </div>
        ...
        <div className={styles.formGroup}>
          <label className={styles.label}>State</label>
          <input className={styles.input} {...register("state", { required: "State is required" })} />
        </div>

        {/** Image **/}
        <div className={styles.formGroup}>
          <label className={styles.label}>Image</label>
          <input type="file" className={styles.fileInput} {...register("image")} />
          {preview && <img src={preview} alt="Preview" width="100" />}
        </div>
...
// ✅ Server-side route protection & fetch school
export async function getServerSideProps({ params, req }) {
  const cookies = req.headers.cookie || "";
  const match = cookies.split(";").map(s => s.trim())
    .find(s => s.startsWith((process.env.COOKIE_NAME || "auth_token") + "="));
